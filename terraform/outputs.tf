...
output "web_app_wait_command" {
  value       = "until curl -Is --max-time 5 http://$(kubectl get ing -n cloudacademy frontend -o jsonpath='{.spec.rules[0].host}')/api/stocks/csv | grep 'HTTP/1.1 200'; do echo preparing...; sleep 5; done; echo; echo -e 'Ready...'"
